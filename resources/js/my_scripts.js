...
function loadStatsPage(){
	...
	var win = 0;
	var loss = 0;
	for(row_num = 2; row_num < table.rows.length; row_num++){

		var homeScore = table.rows[row_num].cells[2].innerHTML;
		homeScore = parseInt(homeScore);
		var oppScore = table.rows[row_num].cells[3].innerHTML;
		oppScore = parseInt(oppScore);

		if(homeScore > oppScore) {
			table.rows[row_num].cells[4].innerHTML = "CU Boulder";
			win++;
		} else {
			table.rows[row_num].cells[4].innerHTML = table.rows[row_num].cells[1].innerHTML;
			loss++;
		}
